...
export class ProductDetailPage {
  ...
  img2;
  term;
  private sub_url = '/wp-json/ordertcg/v1/mobile/get/detail_product';
  data;
  mess;
  mess2;
  dateArray = [];
  ...
  ionViewWillEnter() {
    this.orderId = this.activatedRoute.snapshot.paramMap.get('productId');
    this.getData();
  }
  ...
  getData() {
    this.storage.get('shops').then((shops) => {
      this.storage.get('active_shop').then((index) => {
        ...
        let parameter =
          '?token=' + access_token + '&productId=' + this.orderId;
        ...
        this.http2
          ...
          .then((data) => {
            ...
            if (dt.status === 'success') {
              ...
              console.log(this.data.pool.term_id);
              if (this.data.enable_pool === '1') {
                this.term = this.data.pool.term_id;
              }
  ...
  save() {
    ...
    this.storage.get('shops').then((shops) => {
      this.storage.get('active_shop').then((index) => {
        let access_token = shops[index].access_token;
        let end_url = '/wp-json/ordertcg/v1/mobile/update/product_detail';
        ...
        this.http2
          .post(
            url,
            {
              ...
              productName: this.data.product.post_title,
